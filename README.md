...
> CSV-Transformer - Open source tool by AetherCodeHQ

`cli` `csv` `data` `golang` `open-source` `transformation`

## What is it?

**Csv Transformer** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./CSV-Transformer --path ./target

# With options
./CSV-Transformer --path ./target --format json --output report.json

# Verbose mode
./CSV-Transformer --path ./target --verbose
```
...
### Basic scan
```bash
./CSV-Transformer --path ./my-project
```

### JSON report
```bash
./CSV-Transformer --path ./my-project --format json --output report.json
```
...
- name: Run Csv Transformer
  run: ./CSV-Transformer --path . --format json --output report.json
```
